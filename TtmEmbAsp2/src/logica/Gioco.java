...
public class Gioco extends Thread{
	...
	private ArrayList<StepTopo> stepsTopo=new ArrayList<StepTopo>(); 
	private ArrayList<StepBordo> stepsBordo=new ArrayList<StepBordo>();
	...
	public void run() {
		...
		while(gioca)
		{
			...
			aggiungiStepTopoPesi();
			aggiungiStepBordoPesi();
			
			ScelgoMuro sm=inserisci_ostacolo();
			
			schema[sm.getX()][sm.getY()]=muro;
			Muro m=new Muro(sm.getX(),sm.getY());
			muri.add(m);
			gp.repaint();
...
private void aggiungiStepBordoPesi() {
		// TODO Auto-generated method stub
		Collections.sort(steps,Step.ordina);
		
		for (Step s: steps)
		{
			int diff=steps.size()-steps.indexOf(s);
			StepBordo sb=new StepBordo(s.getX(),s.getY(),diff);
			stepsBordo.add(sb);
		}
	}

private void aggiungiStepTopoPesi() {
	// TODO Auto-generated method stub
	
	Collections.sort(steps,Step.ordinaInverso); //ORDINO INVERSO PER I PESI
	
	for (Step s:steps)
	{
		int diff=steps.size()-steps.indexOf(s);
		StepTopo st=new StepTopo(s.getX(),s.getY(),diff);
		stepsTopo.add(st);
	}
...
//		
		...
		//AGGIUNGO STEPS VERSO IL BORDO
		for (StepBordo sb: stepsBordo)
		{
			try {
				facts.addObjectInput(sb);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		for (StepTopo st: stepsTopo)
		{
			try {
				facts.addObjectInput(st);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		try {
			ASPMapper.getInstance().registerClass(ScelgoMuro.class);
